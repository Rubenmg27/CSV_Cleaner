...
class DataValidator:
    ...
    @staticmethod
    def require_one_row(df: pd.DataFrame, name: str = "DataFrame") -> None:
        """
    ...
    def require_configuration__header_types__columns(
        config: Configuration, df: pd.DataFrame, name: str = "Configuration_header_types"
    ) -> None:
        """
        ...
        if len(config.header_types) != len(df.columns):
            raise ValueError(f"{name} must has the same number of columns than DataFrame line")

    # @staticmethod
    # def require_csvcontent(data: Any, name: str = "data") -> CsvContent:
    #     """
    ...
    #         raise TypeError(f"{name} must has CsvContent type, got {type(data).__name__}")
    #     return data
